        ...
        try {
            ...
            allBanksIds.forEach(bank => {
                ...
                borrowersUsers.forEach(borrower => {
                    NotificationsFacade.create(borrower.borrowerId, `У вас списан платеж по кредиту для банка "${bank.name}": ${borrower.paymentSum / 100}$`);
                });
                ...
                borrowersOrgs.forEach(borrower => {
                    ...
                    NotificationsFacade.create(adminId, `У вас списан платеж по кредиту вашей организации для банка "${bank.name}": ${borrower.paymentSum / 100}$`);
                });
            });
        }
        catch (e) {
